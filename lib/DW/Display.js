...
DW.Display.prototype.movePlayerChar = function(positionX, positionY) {
  ...
  for (var i = 0; i < path.length; i++) {
    ...
    var scope = this;
    tweens[i] = function(ii, sourceX, sourceY, targetX, targetY, iDistance, iFieldX, iFieldY, iMovePlayer) {
      return new TWEEN.Tween( { x: sourceX, y: sourceY } )
        ...
        .onComplete( function () {
          scope.movePlayer = iMovePlayer;
          scope.game.dispatchEvent({
            type: "map-field-reached", content: { position: { x: iFieldX, y: iFieldY } }
          });
      ...
      i, lastX, lastY, nextPosX, nextPosY, distance, positionX, positionY, (i == path.length - 1) ? false : true
    );
    if (i > 0) {
