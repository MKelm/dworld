      ...
      return new TWEEN.Tween( { x: sourceX, y: sourceY } )
        ...
        .onUpdate( function () {
          ...
          if (creature.movement.stop == true) {
            ...
            var x1 = creature.gfx.position.x, y1 = creature.gfx.position.y;
  ...
  for (var i = 0; i < this.mapSprites.length; i++) {
    if (this.mapSprites[i].position.x < minMaxField.min.x || this.mapSprites[i].position.y < minMaxField.min.y ||
      ...
      if (this.mapContainer.children.indexOf(this.mapSprites[i].sprite) !== -1) {
        ...
        this.removeCreature(this.mapSprites[i].position.x, this.mapSprites[i].position.y);
      }
    }
  }
...
DW.Display.prototype.drawCreature = function(fX, fY) {
  var creature = this.game.creatureHandler.getCreatureByPosition(fX, fY);
  if (creature !== null && creature.sprite === null) {
    sprite = new PIXI.Sprite(
      PIXI.Texture.fromFrame("creature_" + creature.handle)
    );
...
DW.Display.prototype.removeCreature = function(fX, fY) {
  var creature = this.game.creatureHandler.getCreatureByPosition(fX, fY);
  if (creature !== null && creature.sprite !== null) {
    this.creaturesContainer.removeChild(creature.sprite);
