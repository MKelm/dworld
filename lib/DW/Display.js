...
DW.Display = function(game) {
  ...
  this.borderGfx = null;
  this.borderHealthText = null;
  this.borderManaText = null;
}
...
DW.Display.prototype.initialize = function() {
  ...
  this.container.scale = {x: dw.pixi.screen.ratio * 2, y: dw.pixi.screen.ratio * 2};
...
DW.Display.prototype.drawBorderContents = function() {
  this.borderGfx.clear();
  var posY = -1 * 4.5 * 50, height = 9 * 50;

  this.borderGfx.lineStyle(0, 0);
  // health bar content
  var healthHeight = 1 / this.game.player.maxHealth * this.game.player.health * height;
  this.borderGfx.beginFill(0xB73320);
  this.borderGfx.drawRect(-1 * 5 * 50, posY + (height - healthHeight), 25, healthHeight);
  this.borderGfx.endFill();

  // mana bar content
  var manaHeight = 1 / this.game.player.maxMana * this.game.player.mana * height;
  this.borderGfx.beginFill(0x2F32A4);
  this.borderGfx.drawRect(4.5 * 50, posY + (height - manaHeight), 25, manaHeight);
  this.borderGfx.endFill();

  // map border
  this.borderGfx.lineStyle(3, 0xFFFFFF);
  this.borderGfx.drawRect(-1 * 4.5 * 50, posY, height, height);
  // health bar border
  this.borderGfx.drawRect(-1 * 5 * 50, posY, 25, height);
  // mana bar boder
  this.borderGfx.drawRect(4.5 * 50, posY, 25, height);

  // health text
  var healthText = "Health " + this.game.player.health;
  if (this.borderHealthText === null) {
    var style = {font: 20 + "px " + "Arial", fill: "FFFFFF"}, sprite = null, scope = this;
    this.borderHealthText = new PIXI.Text(healthText, style);
    this.borderHealthText.position = { x: -1 * 4.5 * 50 - 23, y: posY + height - 10 };
    this.borderHealthText.rotation = Math.radians(-90);
    this.container.addChild(this.borderHealthText);
  } else {
    this.borderHealthText.setText(healthText);
  }

  // mana text
  var manaText = "Mana " + this.game.player.mana;
  if (this.borderManaText === null) {
    var style = {font: 20 + "px " + "Arial", fill: "FFFFFF"}, sprite = null, scope = this;
    this.borderManaText = new PIXI.Text(manaText, style);
    this.borderManaText.position = { x: 4.5 * 50 + 2, y: posY + height - 10 };
    this.borderManaText.rotation = Math.radians(-90);
    this.container.addChild(this.borderManaText);
  } else {
    this.borderManaText.setText(manaText);
  }
}
...
DW.Display.prototype.moveCreature = function(creature) {
  ...
  for (var i = 0; i < creature.movement.path.length; i++) {
    positionX = creature.movement.path[i][0];
    positionY = creature.movement.path[i][1];
...
DW.Display.prototype.drawMapFields = function() {
  ...
  for (var fY = minMaxField.min.y; fY <= minMaxField.max.y; fY++) {
    ...
    for (var fX = minMaxField.min.x; fX <= minMaxField.max.x; fX++) {
      ...
      if (typeof map.loadedFields[fY] != "undefined" &&
          typeof map.loadedFields[fY][fX] != "undefined" &&
