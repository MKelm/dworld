...
DW.Display.prototype.movePlayerChar = function(positionX, positionY) {
  ...
  for (var i = 0; i < path.length; i++) {
    ...
    var scope = this;
    tweens[i] = function(ii, sourceX, sourceY, targetX, targetY, iDistance, iFieldX, iFieldY, isLastField) {
      return new TWEEN.Tween( { x: sourceX, y: sourceY } )
        ...
        .onComplete( function () {
          scope.movePlayer = !isLastField;
          scope.game.dispatchEvent({
            type: "map-field-reached", content: { position: { x: iFieldX, y: iFieldY }, isLastField: isLastField }
          });
...
DW.Display.prototype.moveCreature = function(creature) {
  ...
  if (creature.movePath.length > 0) {
    // if valid path set creature position data to new position
    // todo: update positon in a better way, to avoid air collision between player and creature
    creature.position.x = creature.moveTargetPosition.x;
    creature.position.y = creature.moveTargetPosition.y;
  }

  var tweens = [], lastX = creature.gfx.position.x, lastY = creature.gfx.position.y;
  for (var i = 0; i < creature.movePath.length; i++) {
    positionX = creature.movePath[i].x;
    positionY = creature.movePath[i].y;

    nextPosX = positionX * fWidth;
    nextPosY = positionY * fHeight;

    // get correct animation time by distance and creature speed
    var distance = Math.sqrt(
      Math.pow(lastX - nextPosX, 2) + Math.pow(lastY - nextPosY, 2)
    );
    // move mapContainer to display player char movment in middle of screen
    ...
    tweens[i] = function(ii, sourceX, sourceY, targetX, targetY, iDistance, iFieldX, iFieldY, isLastField) {
      return new TWEEN.Tween( { x: sourceX, y: sourceY } )
        .to(
          { x: targetX, y: targetY }, 1000 * (iDistance / creature.speed)
        )
        .onUpdate( function () {
          creature.gfx.position.x = this.x;
          creature.gfx.position.y = this.y;
        })
        .onComplete( function () {
          if (isLastField == true) {
            creature.resetMove();
      ...
      i, lastX, lastY, nextPosX, nextPosY, distance, positionX, positionY,
      (i == creature.movePath.length - 1) ? true : false
    );
    if (i > 0) {
      tweens[i-1].chain(tweens[i]);
    }
