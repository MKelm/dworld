      ...
      for (var fY = 0; fY < area.fields.length; fY++) {
        ...
        for (var fX = 0; fX < area.fields[fY].length; fX++) {
          ...
          if (area.fields[fY][fX] == 4) {
            ...
            this.game.chestHandler.addChest(fX, fY);
          }
        }
      }
...
DW.Map.prototype.performFieldReachedAction = function(position, isLastField) {
  // determine next action by field type
  switch (this.loadedFields[position.y][position.x]) {
    case 4: // show chest if last field
      if (isLastField === true) {
        this.game.windows.chest.tableContent = this.game.chestHandler.getChestByPosition(
          position.x, position.y
        ).content;
        this.game.windows.chest.show();
      }
      break;
  }
  // stop on fields with a creature
  if (this.game.creatureHandler.getCreatureByPosition(position.x, position.y) !== null) {
    this.game.display.movePlayer = false;
