...
DW.Map.prototype.getMinMaxField = function(playerPosition) {
  ...
  var maxPosition = {
    x: playerPosition.x + this.viewDistance - 1,
    y: playerPosition.y + this.viewDistance - 1
  };
  if (maxPosition.x > this.fieldAmount.x - 1) maxPosition.x = this.fieldAmount.x - 1;
  if (maxPosition.y > this.fieldAmount.y - 1) maxPosition.y = this.fieldAmount.y - 1;
    ...
    for (var areaX = startAreaX; areaX <= endAreaX; areaX++) {
      ...
      for (var fY = 0; fY < area.fields.length; fY++) {
        ...
        for (var fX = 0; fX < area.fields[fY].length; fX++) {
          this.loadedFields[fieldY + fY].push({ type: area.fields[fY][fX], visible: false, sprite: null });
