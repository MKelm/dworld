...
DW.Map.prototype.calculatePath = function (position, targetPosition) {
  // todo: bug on large distances!!!
  var dirX = null, dirY = null, path = [], fields = [], fieldType = null, fieldId = null;

  var path = [], calculate = true;
  console.log("target", targetPosition.x, targetPosition.y);
  do {
    // search valid fields to go
    ...
    fields = [
      { x: position.x + dirX, y: position.y },
      { x: position.x + dirX, y: position.y + dirY },
      { x: position.x, y: position.y + dirY }
    ];
