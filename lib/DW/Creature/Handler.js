...
DW.CreatureHandler.prototype.update = function() {
  ...
  for (var i = 0; i < this.activeCreatures.length; i++) {
    if (this.activeCreatures[i] instanceof DW.Creature) {
      if (this.activeCreatures[i].loadMoveTargetPosition() === true) {
        this.activeCreatures[i].movePath = this.game.map.calculatePath(
          this.activeCreatures[i].position, this.activeCreatures[i].moveTargetPosition
        );
        if (this.activeCreatures[i].movePath.length > 0) {
          this.activeCreatures[i].move = true;
        }
      }
    }
  }
