...
DW.Pixi.prototype.constructor = DW.Pixi;

DW.Pixi.prototype.autoResizeRenderer = function() {
...
DW.Pixi.prototype.animate = function(scope) {
  ...
  if (dw.game.run == true) {
    dw.game.update(dw.game);
  }
  scope.renderer.render(scope.stage);
  TWEEN.update();

  if (dw.userConfig.fps > -1) {
    global.setTimeout(function() { requestAnimFrame(scope.animate.curry(scope)); }, 1000 / dw.userConfig.fps );
  } else {
    requestAnimFrame(scope.animate.curry(scope));
  }
}

DW.Pixi.prototype.setLoader = function(activate) {
  if (activate === true) {
...
DW.Pixi.prototype.loadAssets = function(callback) {
  this.setLoader(true);
  var assetsToLoad = [ "data/gfx/gfx.atlas" ];
...
DW.Pixi.prototype.setFpsCounter = function(keyEvent, scope) {
  ...
  if (typeof scope.fps == "undefined") {
    ...
    if (keyEvent === true && scope.fps[0] < 0) {
      ...
      dw.intervals["fpsCounter"] = setInterval(scope.setFpsCounter.curry(false, scope), 1000);
      keyEvent = false;
    ...
    if (keyEvent === true && scope.fps[0] > -1) {
      clearInterval(dw.intervals["fpsCounter"]);
      scope.fps[1].visible = false;
      scope.fps[0] = -1;
