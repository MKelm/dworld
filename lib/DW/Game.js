...
DW.Game = function() {
  ...
  this.addEventListener('map-area-click', dw.util.getEventListener(this, "handleEvent"));
}
...
DW.Game.prototype.handleEvent = function(scope, event) {
  switch (event.type) {
    case "map-area-click":
      var mouseEvent = event.content.mouse.originalEvent;
      // set player to new location and draw animation
      this.display.drawPlayerChar(
        -1 * (dw.pixi.screen.width/2 - mouseEvent.layerX),
        -1 * (dw.pixi.screen.height/2 - mouseEvent.layerY)
      );
      break;
  }
}