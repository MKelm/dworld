...
DW.Game = function() {
  ...
  this.windows = {
    chest: new DW.DisplayWindowChest(this), // test window, chest
    inventory: null
  };

  // test chest window content
  this.windows.chest.tableContent = this.chestHandler.activeChests[0].content;
...
DW.Game.prototype.handleEvent = function(scope, event) {
  switch (event.type) {
    case "map-field-reached":
      console.log("event map field reached");
