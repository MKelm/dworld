...
DW.Game.prototype.handleEvent = function(scope, event) {
  switch (event.type) {
    case "map-field-reached":
      ...
      if (isLastField === true && scope.map.getTypeByPosition(position.x, position.y) == 4) {
      ...
      } else if (scope.windows.fight.visible == false) {
        var currentCreature = this.creatureHandler.getCreatureByPosition(position.x, position.y);
    ...
    case "window-fight-click":
      ...
      var playerAttackPoints = this.player.makeHit();
      var creatureAttackPoints = this.windows.fight.currentCreature.makeHit();
      this.windows.fight.currentCreature.setHit(playerAttackPoints);
      this.player.setHit(creatureAttackPoints);
      // update fight window or remove creature on fight win
      if (this.windows.fight.currentCreature.hitPoints > 0) {
        this.windows.fight.drawAttackStatus("player", playerAttackPoints);
        this.windows.fight.drawAttackStatus("creature", creatureAttackPoints);
