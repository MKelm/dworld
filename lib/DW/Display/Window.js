...
DW.DisplayWindow.prototype.drawTable = function(rows, cells, cellWidth, cellHeight) {
  var bordersGfx = new PIXI.Graphics(), activeGfx = new PIXI.Graphics(),
      posX = null, posY = 60, absPos = 0, scope = this,
      marginLeft = (this.width - (cells * cellWidth - 1)) / 2, sprite = null;
  bordersGfx.lineStyle(1.5, 0xFFFFFF);
  this.container.addChild(activeGfx);
  for (var r = 0; r < rows; r++) {
    ...
    for (var c = 0; c < cells; c++) {
      ...
      if (typeof this.tableContent[absPos] != "undefined" && this.tableContent[absPos] !== null) {
        ...
        if (this.tableContent[absPos].active == true) {
          activeGfx.beginFill(0xFFD41E);
          activeGfx.drawRect(posX, posY, cellWidth, cellHeight);
          activeGfx.endFill();
        }
      ...
      // cell border
      bordersGfx.drawRect(posX, posY, cellWidth, cellHeight);
      posX += cellWidth;
      absPos++;
    }
    posY += cellHeight;
  }
  this.container.addChild(bordersGfx);
}
...
DW.DisplayWindow.prototype.drawHint = function() {
  if (this.hint.length > 0) {
    if (this.hintGfx === null) {
      ...
      this.hintGfx.setText(this.hint);
    }
  }
}
